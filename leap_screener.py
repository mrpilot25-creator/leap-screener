...
MIN_EBITDA_MARGIN     = 0.15   # 15% EBITDA margin (EBITDA / Revenue)
...
“””
Calculated once per ticker using already-fetched price history.
...
if kind == "revenue_growth":
    # Above 8%; perfect score at 40%+
    return _clamp((value - MIN_REVENUE_GROWTH) / 0.32 * 100, 0, 100)

if kind == "ebitda_margin":
    # Above 15%; perfect score at 45%+
    return _clamp((value - MIN_EBITDA_MARGIN) / 0.30 * 100, 0, 100)
...
if kind == "iv_percentile":
    return 0.0 if value > MAX_IV_PERCENTILE else _clamp(
        (MAX_IV_PERCENTILE - value) / MAX_IV_PERCENTILE * 100, 0, 100
    )
...
# Revenue growth and EBITDA margin each take a share from EPS growth
...
def fetch_ebitda_margin(info: dict):
“””
EBITDA Margin = EBITDA / Total Revenue.
Yahoo Finance provides both fields directly.
Returns a decimal (e.g. 0.32 = 32%) or None if unavailable.
“””
try:
...
```
# Fallback: ebitdaMargins field (sometimes populated directly)
...
“””
Hard-fails a ticker only when data EXISTS and is confirmed below threshold.
Missing data (None) scores 0 but does NOT exclude the ticker —
Yahoo Finance is inconsistent in which fields it populates.

```
Exception: Market Cap missing or below threshold is always a hard fail.
"""
...
# Market cap — always hard fail if missing or too small
if mc is None or mc < MIN_MARKET_CAP:
    label = "no data" if mc is None else f"${mc/1e9:.1f}B < $2B"
    return False, f"MarketCap {label}"

# All other criteria — only fail if data EXISTS and is below threshold
...
if rg is not None and rg < MIN_REVENUE_GROWTH:
    return False, f"RevenueGrowth {rg*100:.1f}% < 8%"

if em is not None and em < MIN_EBITDA_MARGIN:
    return False, f"EBITDAMargin {em*100:.1f}% < 15%"
...
try:
    ...
    if hist.empty or len(hist) < 30:
        print(" X  Insufficient price history")
        return []
    ...
    # IV percentile — calculated once using already-fetched price history
    ...
    for _, opt in calls.iterrows():
        ...
        results.append({
            ...
            "revenue_growth_pct": round(fund["revenue_growth"] * 100, 1)  if fund["revenue_growth"]  else None,
            "ebitda_margin_pct":  round(fund["ebitda_margin"] * 100, 1)   if fund["ebitda_margin"]   else None,
    ...
    rg_s = f"RevGrowth={fund['revenue_growth']*100:.0f}%" if fund["revenue_growth"] is not None else "RevGrowth=?"
    em_s = f"EBITDA={fund['ebitda_margin']*100:.0f}%"    if fund["ebitda_margin"]   is not None else "EBITDA=?"
    print(f" OK  {len(results)} options  {rg_s}  {em_s}  Expiry={expiry}")
...
print(f”  Market Cap         > ${ MIN_MARKET_CAP/1e9:.0f}B”)
print(f”  Avg Daily Volume   > { MIN_AVG_VOLUME/1e6:.0f}M shares”)
print(f”  Inst. Ownership    > { MIN_INST_OWNERSHIP*100:.0f}%”)
print(f”  EPS Growth         > +{ MIN_EPS_GROWTH*100:.0f}%”)
print(f”  Revenue Growth     > +{ MIN_REVENUE_GROWTH*100:.0f}%”)
print(f”  EBITDA Margin      > { MIN_EBITDA_MARGIN*100:.0f}%”)
print()
...
run_meta = {
    ...
    "screening_parameters": {
        ...
        "rsi_upper":                RSI_UPPER,
        "price_vs_200ma_range":     f"+-{PRICE_VS_200MA_RANGE*100:.0f}%",
